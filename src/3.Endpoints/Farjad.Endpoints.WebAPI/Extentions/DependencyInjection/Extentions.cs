...
{
    public static IServiceCollection AddFarjadDependencies(this IServiceCollection services,
        params string[] assemblyNamesForSearch)
    {
        var assemblies = GetAssemblies(assemblyNamesForSearch);
        services.AddFarjadApplicationServices(assemblies).AddFarjadDataAccess(assemblies).AddFarjadUntilityServices().AddCustomDependecies(assemblies);
        return services;
    }
